...
class CanaleCommand extends UniversiboCommand 
{
	...
	function getRequestIdCanale()
	{
		if (!array_key_exists('id_canale', $_GET ) )
		{
			...
			else Error::throwError(_ERROR_DEFAULT,array('id_utente' => $this->sessionUser->getIdUtente(), 'msg'=>'il parametro id_canale non ? specificato nella richiesta','file'=>__FILE__,'line'=>__LINE__));
		}

		if (!ereg('^([0-9]+)$', $_GET['id_canale'] ) )
		{
			Error::throwError(_ERROR_DEFAULT,array('id_utente' => $this->sessionUser->getIdUtente(), 'msg'=>'il parametro id_canale ? sintatticamente non valido','file'=>__FILE__,'line'=>__LINE__));
		}
	...
	function _setUpCanaleCanale()
	{
		...
		if ( $this->requestCanale === false ) 
			Error::throwError(_ERROR_DEFAULT,array('id_utente' => $this->sessionUser->getIdUtente(), 'msg'=>'Il canale richiesto non ? presente','file'=>__FILE__,'line'=>__LINE__));
		
		$canale =& $this->getRequestCanale();
		$user =& $this->getSessionUser();
		
		if ( ! $canale->isGroupAllowed( $user->getGroups() ) )
			Error::throwError(_ERROR_DEFAULT, array('id_utente' => $this->sessionUser->getIdUtente(), 'msg'=>'Non ti ? permesso l\'accesso al canale selezionato, la sessione potrebbe essere scaduta','file'=>__FILE__,'line'=>__LINE__ ) );
