...
//		
		...
		if (!($user->isAdmin() || $referente || ($moderatore && $autore)))
			Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUser(), 'msg' => "Non hai i diritti per modificare il link\n La sessione potrebbe essere scaduta", 'file' => __FILE__, 'line' => __LINE__));
...
//		}
		...
		if (array_key_exists('f31_submit', $_POST))
		{
			...
			if($f31_accept === true)
			{
				$linkItem = new Link($_GET['id_link'], $id_canale, $user->getIdUser(), $f31_URI, $f31_Label, $f31_Description );
				$linkItem->updateLink();
