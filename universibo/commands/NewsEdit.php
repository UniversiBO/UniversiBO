 ...
 */
	function execute()
	{
		...
		if (!($user->isAdmin() || $referente || ($moderatore && $autore)))
			Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => "Non hai i diritti per modificare la notizia\n La sessione potrebbe essere scaduta", 'file' => __FILE__, 'line' => __LINE__));
...
//		}
		...
		if (array_key_exists('f8_submit', $_POST))
		{
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il formato del campo giorno di inserimento non ? valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f8_accept = false;
				$checkdate_ins = false;
			}
			else
			...
			if (array_key_exists('f8_scadenza', $_POST))
			{
				...
				{
					Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'La data di scadenza ? minore della data di inserimento', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f8_accept = false;
				}
