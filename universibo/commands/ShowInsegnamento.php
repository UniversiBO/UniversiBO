...
/*
		$template->assign('ins_langObiettiviEsame','Obiettivi del corso');
		if ($info_didattica->getObiettiviEsameLink() == '' && $info_didattica->getObiettiviEsame() == '' )
			$template->assign('ins_langObiettiviEsameLink','Obiettivi del corso');
		elseif ($info_didattica->getObiettiviEsameLink() != '' && $info_didattica->getObiettiviEsame() == '' )
			$template->assign('ins_langObiettiviEsameLink', $info_didattica->getObiettiviEsameLink() );
		else
			'index.php?do=ShowInfoDidattica&id_canale='.$id_canale.'#obiettivi';
		
		
		
		$template->assign('ins_langProgramma','Programma d\'esame');
		if ($info_didattica->getProgrammaLink() == '' && $info_didattica->getProgramma() == '' )
			$template->assign('ins_langProgrammaLink','');
		elseif ($info_didattica->getProgrammaLink() != '' && $info_didattica->getProgramma() == '' )
			$template->assign('ins_langProgrammaLink', $info_didattica->getProgrammaLink() );
		else
			'index.php?do=ShowInfoDidattica&id_canale='.$id_canale.'#programma';
		
		$template->assign('ins_langMateriale','Materiale didattico e 
testi consigliati');
		if ($info_didattica->getTestiConsigliatiLink() == '' && $info_didattica->getTestiConsigliati() == '' )
			$template->assign('ins_langMaterialeLink','');
		elseif ($info_didattica->getTestiConsigliatiLink() != '' && $info_didattica->getTestiConsigliati() == '' )
			$template->assign('ins_langMaterialeLink', $info_didattica->getTestiConsigliatiLink() );
		else
			'index.php?do=ShowInfoDidattica&id_canale='.$id_canale.'#modalita';


		$template->assign('ins_langModalita','Modalità d\'esame');
		...
		elseif ($info_didattica->getModalitaLink() != '' && $info_didattica->getModalita() == '' )
			$template->assign('ins_langModalitaLink', $info_didattica->getModalitaLink() );
		else
			'index.php?do=ShowInfoDidattica&id_canale='.$id_canale.'#modalita';
		
		
		$template->assign('ins_langAppelli','Appelli d\'esame');
		if ($info_didattica->getAppelliLink() == '' && $info_didattica->getAppelli() == '' )
			$template->assign('ins_langAppelliLink','');
		elseif ($info_didattica->getAppelliLink() != '' && $info_didattica->getAppelli() == '' )
			$template->assign('ins_langAppelliLink', $info_didattica->getAppelliLink() );
		else
			'index.php?do=ShowInfoDidattica&id_canale='.$id_canale.'#appelli';
		//$template->assign('ins_langMaterialeLink','Programma d\'esame');
*/
