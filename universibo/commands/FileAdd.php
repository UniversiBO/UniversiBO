	...
	function execute() {
		...
		*/
		...
		if (array_key_exists('f12_submit', $_POST)) {
			...
			if (!array_key_exists('f12_titolo', $_POST) || !array_key_exists('f12_data_ins_gg', $_POST) || !array_key_exists('f12_data_ins_mm', $_POST) || !array_key_exists('f12_data_ins_aa', $_POST) || !array_key_exists('f12_data_ins_ora', $_POST) || !array_key_exists('f12_data_ins_min', $_POST) || !array_key_exists('f12_data_scad_gg', $_POST) || !array_key_exists('f12_data_scad_mm', $_POST) || !array_key_exists('f12_data_scad_aa', $_POST) || !array_key_exists('f12_data_scad_ora', $_POST) || !array_key_exists('f12_data_scad_min', $_POST) || !array_key_exists('f12_testo', $_POST)) {
			...
			if (array_key_exists('f12_scadenza', $_POST)) {

				$f12_scadenza = true;
				$checkdate_scad = true;
				//data_scad_gg
				if (!ereg('^([0-9]{1,2})$', $_POST['f12_data_scad_gg'])) {
					Error :: throw (_ERROR_NOTICE, array ('msg' => 'Il formato del campo giorno di inserimento non \u00e8 valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f12_accept = false;
					$checkdate_scad = false;
				} else
					$f12_data_scad_gg = $_POST['f12_data_scad_gg'];

				//f12_data_scad_mm
				if (!ereg('^([0-9]{1,2})$', $_POST['f12_data_scad_mm'])) {
					Error :: throw (_ERROR_NOTICE, array ('msg' => 'Il formato del campo mese di inserimento non \u00e8 valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f12_accept = false;
					$checkdate_scad = false;
				} else
					$f12_data_scad_mm = $_POST['f12_data_scad_mm'];

				//f12_data_scad_aa
				if (!ereg('^([0-9]{4})$', $_POST['f12_data_scad_aa'])) {
					Error :: throw (_ERROR_NOTICE, array ('msg' => 'Il formato del campo anno di inserimento non \u00e8 valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f12_accept = false;
					$checkdate_scad = false;
				}
				elseif ($_POST['f12_data_scad_aa'] < 1970 || $_POST['f12_data_scad_aa'] > 2032) {
					Error :: throw (_ERROR_NOTICE, array ('msg' => 'Il campo anno di inserimento deve essere compreso tra il 1970 e il 2032', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f12_accept = false;
					$checkdate_scad = false;
				} else
					$f12_data_scad_aa = $_POST['f12_data_scad_aa'];

				//f12_data_scad_ora
				if (!ereg('^([0-9]{1,2})$', $_POST['f12_data_scad_ora'])) {
					Error :: throw (_ERROR_NOTICE, array ('msg' => 'Il formato del campo ora di inserimento non \u00e8 valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f12_accept = false;
				}
				elseif ($_POST['f12_data_scad_ora'] < 0 || $_POST['f12_data_scad_ora'] > 23) {
					Error :: throw (_ERROR_NOTICE, array ('msg' => 'Il campo ora di inserimento deve essere compreso tra 0 e 23', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f12_accept = false;
				} else
					$f12_data_scad_ora = $_POST['f12_data_scad_ora'];

				//f12_data_scad_min
				if (!ereg('^([0-9]{1,2})$', $_POST['f12_data_scad_min'])) {
					Error :: throw (_ERROR_NOTICE, array ('msg' => 'Il formato del campo minuto di inserimento non \u00e8 valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f12_accept = false;
				}
				elseif ($_POST['f12_data_scad_min'] < 0 || $_POST['f12_data_scad_min'] > 59) {
					Error :: throw (_ERROR_NOTICE, array ('msg' => 'Il campo ora di inserimento deve essere compreso tra 0 e 59', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f12_accept = false;
				} else
					$f12_data_scad_min = $_POST['f12_data_scad_min'];

				if ( $checkdate_scad == true && !checkdate($_POST['f12_data_scad_mm'], $_POST['f12_data_scad_gg'], $_POST['f12_data_scad_aa']))
				{
				...
				$data_scadenza = mktime($_POST['f12_data_scad_ora'], $_POST['f12_data_scad_min'], "0", $_POST['f12_data_scad_mm'], $_POST['f12_data_scad_gg'], $_POST['f12_data_scad_aa']);
			...
			//testo	
			if (strlen($_POST['f12_testo']) > 3000) {
			...
			elseif ($_POST['f12_testo'] == '') {
				...
				$f12_testo = $_POST['f12_testo'];

			//flag urgente
			if (array_key_exists('f12_urgente', $_POST)) {
				$f12_urgente = true;
			}
		...
		$template->assign('f12_data_scad_gg', $f12_data_scad_gg);
		$template->assign('f12_data_scad_mm', $f12_data_scad_mm);
		$template->assign('f12_data_scad_aa', $f12_data_scad_aa);
		$template->assign('f12_data_scad_ora', $f12_data_scad_ora);
		$template->assign('f12_data_scad_min', $f12_data_scad_min);
		$template->assign('f12_testo', $f12_testo);
		$template->assign('f12_urgente', $f12_urgente);
		$template->assign('f12_scadenza', $f12_scadenza);
