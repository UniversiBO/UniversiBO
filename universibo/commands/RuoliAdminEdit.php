...
class RuoliAdminEdit extends UniversiboCommand {
	...
	function execute() {
		...
		if (!array_key_exists('id_utente', $_GET) || !ereg('^([0-9]{1,9})$', $_GET['id_utente']))
			Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => 'L\'id dell\'utente richiesto non ? valido', 'file' => __FILE__, 'line' => __LINE__));
		...
		if (!$user->isAdmin() && !$referente )
			Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => "Non hai i diritti per modificare i diritti degli utenti su questa pagina.\nLa sessione potrebbe essere scaduta.", 'file' => __FILE__, 'line' => __LINE__));
		
		if (!$user->isAdmin() && $user->getIdUser() == $target_user->getIdUser() )
			Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Non ? permesso modificare i propri diritti in una pagina', 'file' => __FILE__, 'line' => __LINE__));
