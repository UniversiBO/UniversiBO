	...
	function execute() 
	{
		...
		if ($user->isGroupAllowed( $file->getPermessiDownload() ))
		{
			...
			if (!file_exists($nomeFile)) 
				Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUtente(), 'msg'=>'Impossibile trovare il file richiesto, contattare l\'amministratore del sito','file'=>__FILE__,'line'=>__LINE__ ));
			if ( md5_file($nomeFile) != $file->getHashFile() ) 
				Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUtente(), 'msg'=>'Il file richiesto risulta corrotto, contattare l\'amministratore del sito','file'=>__FILE__,'line'=>__LINE__ ));
		...
		Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Non ? permesso eseguire il download del file.
