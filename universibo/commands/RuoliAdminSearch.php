...
class RuoliAdminSearch extends UniversiboCommand {
	...
	function execute() {
		...
		if (!$user->isAdmin() && !$referente )
			Error :: throwError(_ERROR_DEFAULT, array ('msg' => "Non hai i diritti per modificare i diritti degli utenti su questa pagina.\nLa sessione potrebbe essere scaduta.", 'file' => __FILE__, 'line' => __LINE__));
		...
		if (array_key_exists('f16_submit', $_POST)  )
		{
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Specificare almeno uno dei due criteri di ricerca', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f16_accept = false;
			}	
