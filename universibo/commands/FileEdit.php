...
//		$id_canale = $canale->getIdCanale();
		...
		if (array_key_exists('f13_submit', $_POST)) 
		{
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUser(), 'msg' => 'Il tipo inviato contiene un valore non ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f13_accept = false;
			}
			...
			if (array_key_exists('f13_password_enable', $_POST))
			{
				...
				{ 
					Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUser(), 'msg' => 'La password e il campo di verifica non corrispondono', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' =>& $template));
					$f13_accept = false;
				}
