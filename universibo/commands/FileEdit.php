...
//		$id_canale = $canale->getIdCanale();
		...
		if (array_key_exists('id_canale', $_GET))
		{
			...
			if (!($user->isAdmin() || $referente || ($moderatore && $autore)))
				Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => "Non hai i diritti per modificare il file\n La sessione potrebbe essere scaduta", 'file' => __FILE__, 'line' => __LINE__));
		...
		if (array_key_exists('f13_submit', $_POST)) 
		{
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il formato del campo tipo non ? ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f13_accept = false;
			}
			elseif ( !array_key_exists($_POST['f13_tipo'], $f13_tipi) )
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il tipo inviato contiene un valore non ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f13_accept = false;
			}
			...
			else 
			{
				...
				{
					Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il valore dei diritti di download non ? ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' =>& $template));
					$f13_accept = false;
				}
				$f13_permessi_download = $_POST['f13_permessi_download'];
			...
			if (array_key_exists('f13_password_enable', $_POST))
			{
				...
				{ 
					Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'La password inserita ? vuota', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' =>& $template));
					$f13_accept = false;
				}
				elseif( $file->getPassword() != null && $_POST['f13_password'] == '')
				{ 
