...
class NewPasswordDocente extends UniversiboCommand 
{
	function execute()
	{
		...
		if (!$session_user->isAdmin())
			Error::throwError(_ERROR_DEFAULT,array('id_utente' => $session_user->getIdUser(), 'msg'=>'La generazione di una nuova password dei docenti ? possibile solo ad utenti amministratori.'."\n".'La sessione potrebbe essere scaduta, eseguire il login','file'=>__FILE__,'line'=>__LINE__));
		
		
		$username_allowed = explode(';',$fc->getAppSetting('passwordDocentiAdmin'));
		if (!in_array($session_user->getUsername(), $username_allowed ))
			Error::throwError(_ERROR_DEFAULT,array('id_utente' => $session_user->getIdUser(), 'msg'=>'La generazione di una nuova password dei docenti ? possibile solo a '.implode(', ',$username_allowed)."\n".'La sessione potrebbe essere scaduta, eseguire il login','file'=>__FILE__,'line'=>__LINE__));
		
		if ( !array_key_exists('id_utente', $_GET)  || !ereg('^([0-9]{1,9})$', $_GET['id_utente']))
			Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $session_user->getIdUser(), 'msg' => 'L\'id dell\'utente richiesto non ? valido', 'file' => __FILE__, 'line' => __LINE__));
...
//							'In ogni caso non comunicate mai le vostre password di ateneo, lo staff non ? tenuto a conoscerle');
		...
		if ( $f21_accept == true )
		{
			...
			if ($docente->updatePasswordHash(User::passwordHashFunction($randomPassword),true) == false)
				Error::throwError(_ERROR_DEFAULT,array('id_utente' => $session_user->getIdUser(), 'msg'=>'Si ? verificato un errore durante l\'aggiornamento della password','file'=>__FILE__,'line'=>__LINE__));
			...
			if(!$mail->Send()) 
				Error::throwError(_ERROR_DEFAULT,array('id_utente' => $session_user->getIdUser(), 'msg'=>'Attenzione!!! La password ? stata aggiornata su database ma potrebbe essersi verificato un errore durante la spedizione dell\'email!!'."\nPassword: $randomPassword",'file'=>__FILE__,'line'=>__LINE__));
