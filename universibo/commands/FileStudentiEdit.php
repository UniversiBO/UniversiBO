...
//		$id_canale = $canale->getIdCanale();
		...
		if (array_key_exists('id_canale', $_GET))
		{
			...
			if (!($autore))
				Error :: throwError(_ERROR_DEFAULT, array ('msg' => "Non hai i diritti per modificare il file\n La sessione potrebbe essere scaduta", 'file' => __FILE__, 'line' => __LINE__));
...
//		}
		...
		if (array_key_exists('f24_submit', $_POST)) 
		{
			...
			elseif ($_POST['f24_data_ins_aa'] < 1970 || $_POST['f24_data_ins_aa'] > 2032) {
			...
			elseif ($_POST['f24_data_ins_ora'] < 0 || $_POST['f24_data_ins_ora'] > 23) {
			...
			if ( $checkdate_ins == true && !checkdate($_POST['f24_data_ins_mm'], $_POST['f24_data_ins_gg'], $_POST['f24_data_ins_aa']))
			{
			...
			$f24_data_inserimento = mktime($_POST['f24_data_ins_ora'], $_POST['f24_data_ins_min'], "0", $_POST['f24_data_ins_mm'], $_POST['f24_data_ins_gg'], $_POST['f24_data_ins_aa']);
...
//			}
			...
			else $f24_categoria = $_POST['f24_categoria'];
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Il tipo inviato contiene un valore non ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f24_accept = false;
			}
			else $f24_tipo = $_POST['f24_tipo'];
			...
			if ($f24_accept == true) 
			{
				...
				if ($edit_password)
					$file->setPassword( ($f24_password == null) ? $f24_password : FileItem::passwordHashFunction($f24_password));
		...
		$template->assign('fileEdit_flagCanali', (count($f24_canale)) ? 'true' : 'false');
...
//		$template->assign('f24_password_enable', ($f24_password_enable) ? 'true' : 'false' );
		...
		$template->assign('f24_data_ins_gg', $krono->k_date('%j',$f24_data_inserimento));
		$template->assign('f24_data_ins_mm', $krono->k_date('%m',$f24_data_inserimento));
		$template->assign('f24_data_ins_aa', $krono->k_date('%Y',$f24_data_inserimento));
