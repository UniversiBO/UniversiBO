...
class NewsAdd extends CanaleCommand {
	...
	function execute() {
		...
		if (!($user->isAdmin() || $referente || $moderatore)) 
			Error :: throwError(_ERROR_DEFAULT, array ('msg' => "Non hai i diritti per inserire una notizia\n La sessione potrebbe essere scaduta", 'file' => __FILE__, 'line' => __LINE__));
		...
		*/
		...
		if (array_key_exists('f7_submit', $_POST)) {
			...
			if (array_key_exists('f7_scadenza', $_POST)) {
				...
				{
					Error :: throwError(_ERROR_NOTICE, array ('msg' => 'La data di scadenza specificata non esiste', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f7_accept = false;
				}
				...
				{
					Error :: throwError(_ERROR_NOTICE, array ('msg' => 'La data di scadenza non pu? essere inferiore alla data di inserimento', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f7_accept = false;
				}
