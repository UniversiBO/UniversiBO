...
class FileStudentiAdd extends UniversiboCommand {
	...
	function execute() {
		...
		if ($user->isOspite())
		{
			Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => "Per questa operazione bisogna essere registrati\n la sessione potrebbe essere terminata", 'file' => __FILE__, 'line' => __LINE__));
		}		
...
*/
		...
		if (array_key_exists('id_canale', $_GET))
		{
			if (!ereg('^([0-9]{1,9})$', $_GET['id_canale']))
				Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => 'L\'id del canale richiesto non é valido', 'file' => __FILE__, 'line' => __LINE__));

			$canale = & Canale::retrieveCanale($_GET['id_canale']);
			
			if ($canale->getServizioFiles() == false) 
				Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => "Il servizio files é disattivato", 'file' => __FILE__, 'line' => __LINE__));
...
//		
		if (array_key_exists('id_canale', $_GET))
		 {
			$diritti = !$user->isOspite() && $canale->isGroupAllowed($user->getGroups());
			if (!$diritti) 
				Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUtente(), 'msg' => "Non hai i diritti per inserire un file\n La sessione potrebbe essere scaduta", 'file' => __FILE__, 'line' => __LINE__));
		 }
		...
		if (array_key_exists('f23_submit', $_POST)) 
		{
			...
			if (strlen($_POST['f23_titolo']) > 150) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il titolo deve essere inferiore ai 150 caratteri', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			elseif ($_POST['f23_titolo'] == '') {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il titolo deve essere inserito obbligatoriamente', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			} else
			...
			if (!ereg('^([0-9]{1,2})$', $_POST['f23_data_ins_gg'])) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il formato del campo giorno di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
				$checkdate_ins = false;
			} else
			...
			if (!ereg('^([0-9]{1,2})$', $_POST['f23_data_ins_mm'])) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il formato del campo mese di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
				$checkdate_ins = false;
			} else
			...
			if (!ereg('^([0-9]{4})$', $_POST['f23_data_ins_aa'])) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il formato del campo anno di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
				$checkdate_ins = false;
			}
			elseif ($_POST['f23_data_ins_aa'] < 1970 || $_POST['f23_data_ins_aa'] > 2032) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il campo anno di inserimento deve essere compreso tra il 1970 e il 2032', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
				$checkdate_ins = false;
			} else
			...
			if (!ereg('^([0-9]{1,2})$', $_POST['f23_data_ins_ora'])) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il formato del campo ora di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			elseif ($_POST['f23_data_ins_ora'] < 0 || $_POST['f23_data_ins_ora'] > 23) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il campo ora di inserimento deve essere compreso tra 0 e 23', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			} else
			...
			if (!ereg('^([0-9]{1,2})$', $_POST['f23_data_ins_min'])) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il formato del campo minuto di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			elseif ($_POST['f23_data_ins_min'] < 0 || $_POST['f23_data_ins_min'] > 59) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il campo ora di inserimento deve essere compreso tra 0 e 59', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			} else
				$f23_data_ins_min = $_POST['f23_data_ins_min'];

			if ( $checkdate_ins == true && !checkdate($_POST['f23_data_ins_mm'], $_POST['f23_data_ins_gg'], $_POST['f23_data_ins_aa']))
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'La data di inserimento specificata non esiste', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			...
			if (strlen($_POST['f23_abstract']) > 3000) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'La descrizione/abstract del file deve essere inferiore ai 3000 caratteri', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			elseif ($_POST['f23_abstract'] == '') {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'La descrizione/abstract del file deve essere inserita obbligatoriamente', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			} else
				$f23_testo = $_POST['f23_abstract'];
			...
			{	
				...
				{
					Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Si possono inserire al massimo 4 parole chiave', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f23_accept = false;
				}
				else 
				{
					foreach($parole_chiave as $parola)
					{
						if (strlen($parola > 40))
						{
							Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'La lunghezza massima di una parola chiave ? di 40 caratteri', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
							$f23_accept = false;
						}
						else
						{
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il formato del campo categoria non é ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			elseif ( !array_key_exists($_POST['f23_categoria'], $f23_categorie) )
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'La categoria inviata contiene un valore non ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'I permessi di download non sono validi', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			elseif ( $user->isAdmin() ) 
			{
				...
				{
					Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il valore dei diritti di download non ? ammessibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' =>& $template));
					$f23_accept = false;
				}
			...
			{	
				if ($_POST['f23_permessi_download'] != USER_ALL && $_POST['f23_permessi_download'] != (USER_STUDENTE | USER_DOCENTE | USER_TUTOR | USER_PERSONALE | USER_COLLABORATORE | USER_ADMIN ) )
				{
					Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUtente(), 'msg' => 'Il valore dei diritti di download non ? ammessibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' =>& $template));
					$f23_accept = false;
				}
...
//			}
			...
			if (array_key_exists('id_canale', $_GET))
				...
				$f23_permessi_visualizza = USER_ALL;
			...
			{
				Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUtente(), 'msg'=>'E\' severamente vietato inserire file con estensione .php','file'=>__FILE__,'line'=>__LINE__));
				$f23_accept = false;
			}	
			elseif (!is_uploaded_file($_FILES['f23_file']['tmp_name'])) 
			{
				Error :: throwError(_ERROR_NOTICE, array('id_utente' => $user->getIdUtente(), 'msg' => 'Non e\' stato inviato nessun file', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			...
			if ($f23_accept == true) 
			{
				...
				$nome_file = FileItem::normalizzaNomeFile($_FILES['f23_file']['name']);
				...
				if ( move_uploaded_file($_FILES['f23_file']['tmp_name'],$frontcontroller->getAppSetting('filesPath').$nomeFile ) === false )
				{
					$db->rollback();
					Error :: throwError(_ERROR_DEFAULT, array('id_utente' => $user->getIdUtente(), 'msg' => 'Errore nella copia del file', 'file' => __FILE__, 'line' => __LINE__));
				}
...
//				ignore_user_abort(0);
		...
		$template->assign('fileAdd_flagCanali', (count($f23_canale)) ? 'true' : 'false');
...
//		$template->assign('f23_password', $f23_password);
		...
		$template->assign('f23_data_ins_gg', $krono->k_date('%j',$f23_data_inserimento));
		$template->assign('f23_data_ins_mm', $krono->k_date('%m',$f23_data_inserimento));
		$template->assign('f23_data_ins_aa', $krono->k_date('%Y',$f23_data_inserimento));
