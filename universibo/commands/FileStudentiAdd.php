...
*/
		...
		if (array_key_exists('id_canale', $_GET))
		{
			if (!ereg('^([0-9]{1,9})$', $_GET['id_canale']))
				Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUser(), 'msg' => 'L\'id del canale richiesto non é valido', 'file' => __FILE__, 'line' => __LINE__));

			$canale = & Canale::retrieveCanale($_GET['id_canale']);
			
			if ($canale->getServizioFiles() == false) 
				Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUser(), 'msg' => "Il servizio files é disattivato", 'file' => __FILE__, 'line' => __LINE__));
...
//		
		...
		if (array_key_exists('f23_submit', $_POST)) 
		{
			...
			if (!ereg('^([0-9]{4})$', $_POST['f23_data_ins_aa'])) {
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUser(), 'msg' => 'Il formato del campo anno di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
				$checkdate_ins = false;
			}
			...
			{	
				...
				{
					Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUser(), 'msg' => 'Si possono inserire al massimo 4 parole chiave', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f23_accept = false;
				}
				else 
				{
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('id_utente' => $user->getIdUser(), 'msg' => 'La categoria inviata contiene un valore non ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
...
//			}
			...
			{
				Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUser(), 'msg'=>'E\' severamente vietato inserire file con estensione .php','file'=>__FILE__,'line'=>__LINE__));
				$f23_accept = false;
			}	
