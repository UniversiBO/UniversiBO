...
class FileStudentiAdd extends UniversiboCommand {
	...
	function execute() {
		...
		if ($user->isOspite())
		{
			Error :: throwError(_ERROR_DEFAULT, array ('msg' => "Per questa operazione bisogna essere registrati\n la sessione potrebbe essere terminata", 'file' => __FILE__, 'line' => __LINE__));
		}		
...
//		
		if (array_key_exists('id_canale', $_GET))
		 {
			...
			if (!$diritti) 
				Error :: throwError(_ERROR_DEFAULT, array ('msg' => "Non hai i diritti per inserire un file\n La sessione potrebbe essere scaduta", 'file' => __FILE__, 'line' => __LINE__));
		 }
		...
		if (array_key_exists('f23_submit', $_POST)) 
		{
			...
			if (!ereg('^([0-9]{1,2})$', $_POST['f23_data_ins_gg'])) {
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Il formato del campo giorno di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
				$checkdate_ins = false;
			} else
			...
			if (!ereg('^([0-9]{1,2})$', $_POST['f23_data_ins_mm'])) {
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Il formato del campo mese di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
				$checkdate_ins = false;
			} else
			...
			if (!ereg('^([0-9]{4})$', $_POST['f23_data_ins_aa'])) {
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Il formato del campo anno di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
				$checkdate_ins = false;
			}
			...
			if (!ereg('^([0-9]{1,2})$', $_POST['f23_data_ins_ora'])) {
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Il formato del campo ora di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			...
			if (!ereg('^([0-9]{1,2})$', $_POST['f23_data_ins_min'])) {
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Il formato del campo minuto di inserimento non é valido', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'La data di inserimento specificata non esiste', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			...
			{	
				...
				{
					Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Si possono inserire al massimo 4 parole chiave', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
					$f23_accept = false;
				}
				else 
				{
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'Il formato del campo categoria non é ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			elseif ( !array_key_exists($_POST['f23_categoria'], $f23_categorie) )
			{
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'La categoria inviata contiene un valore non ammissibile', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			...
			{
				Error :: throwError(_ERROR_NOTICE, array ('msg' => 'I permessi di download non sono validi', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			elseif ( $user->isAdmin() ) 
			{
...
//			}
			...
			{
				Error::throwError(_ERROR_DEFAULT,array('msg'=>'E\' severamente vietato inserire file con estensione .php','file'=>__FILE__,'line'=>__LINE__));
				$f23_accept = false;
			}	
			elseif (!is_uploaded_file($_FILES['f23_file']['tmp_name'])) 
			{
				Error :: throwError(_ERROR_NOTICE, array('msg' => 'Non e\' stato inviato nessun file', 'file' => __FILE__, 'line' => __LINE__, 'log' => false, 'template_engine' => & $template));
				$f23_accept = false;
			}
			...
			if ($f23_accept == true) 
			{
				...
				{
					$db->rollback();
					Error :: throwError(_ERROR_DEFAULT, array('msg' => 'Errore nella copia del file', 'file' => __FILE__, 'line' => __LINE__));
				}
