	...
	<div class="elencoFile">
		...
		{foreach name=showmyfiletitoli from=$showAllFilesStudentiTitoli_fileList item=temp_file}
			<tr class="{if ($smarty.foreach.showmyfiletitoli.iteration % 2) == 0}odd{else}even{/if}">
				...
				<tr><td class="{if ($smarty.foreach.showmyfiletitoli.iteration % 2) == 0}odd{else}even{/if}" colspan="7" align="center"><a href={$temp_file.canaleLink|escape:"htmlall"}>{$temp_file.canaleTitolo|escape:"htmlall"} </a></td></tr>
			</tr>
			{/foreach}
		</table>
	</div>
