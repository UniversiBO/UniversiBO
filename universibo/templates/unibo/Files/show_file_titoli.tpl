	...
	{if $showFileTitoli_langFileAvailableFlag=="true"}
		{foreach name=listacategorie from=$showFileTitoli_fileList item=temp_categoria}
			...
			{foreach name=listafile from=$temp_categoria.file item=temp_file}
				<p class="{cycle values="even,odd"}">&gt;&nbsp;<a href="{$temp_file.show_info_uri|escape:"htmlall"}">{$temp_file.titolo|escape:"htmlall"|nl2br|truncate}</a>&nbsp;{if $temp_file.nuova=="true"}&nbsp;&nbsp;<img src="tpl/black/icona_new.gif" width="21" height="9" alt="!NEW!" />{/if}
