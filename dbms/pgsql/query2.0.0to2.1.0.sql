...
[*]Cliccare la voce "Inserisci il tuo contributo" nel nel box "Contributi degli studenti", apparirà una nuova pagina "Aggiungi un nuovo file" nella quale dovrai completare i seguenti campi:
[list][*][b]File[/b]:  inserire il percorso interno al tuo computer del file che vuoi caricare, puoi aiutarti con il pulsante "Sfoglia" (o \'Browse\') e cercare la cartella del pc in cui é contenuto il file, selezionarlo e cliccare su \'Apri\' (o \'Open\', a seconda del sistema operativo utilizzato).  Per rendere piú veloce il processo di upload (e conseguentemente di download da parte degli utenti che desidereranno scaricare il file sul loro computer) é consigliabile comprimere il file prima di inviarlo.
NB: si puó caricare [b]un solo file alla volta[/b], quindi per mettere on line piú files bisogna ripetere la procedura.
[*][b]Titolo:[/b] inserire un titolo significativo per il file, in modo che si intuisca facilmente il contenuto. Il titolo comparirà nella lista dei file inseriti dagli studenti
[*][b]Descrizione:[/b] scrivere una descrizione esauriente del contenuto del file.
[*][b]Parole chiave:[/b] scrivere al massimo 4 parole, separate da un Enter/Invio, per consentire una più facile individuazione del file da parte di altri utenti, mediante l\'uso di un motore di ricerca file.
[*][b]Categoria:[/b] scegliere la tipologia del file che stai inserendo tramite il menù a discesa. Se i file non appartiene a nessuna delle categorie presenti, lasciare "altro".
[*][b]Data e ora d\'inserimento.[/b] questi campi vengono completati automaticamente; se lo si desidera, li si può cambiare: può essere molto utile nel caso in cui si voglia che il file non sia visibile prima di una certa data e ora.
[*][b]Permessi download:[/b]  selezionando dal menù a discesa la voce "Solo iscritti", il file sarà scaricabile dai soli utenti registrati, mentre selezionando "Tutti", il file sarà scaricabile da chiunque fruisce del sito.
[*][b]Il file verrà inserito nella pagina :[/b] qui viene visualizzato il nome dell\'insegnamento/corso in cui il file verrà inserito, il contenuto non è modificabile e viene impostato in automatico
[/list]
[*]Cliccare su invia.
[/list]Se la procedura é stata completata con successo, verrà visualizzata una pagina di conferma del  corretto inserimento del file. Cliccando sul link "Torna a..." si ritorna alla pagina dove é stata inserito il file. Il titolo del nuovo file inserito comparirà nel Box \'Contributi degli studenti\'.
',1117109715,320);

INSERT INTO help (titolo,contenuto,ultima_modifica,indice) VALUES ('Come faccio a modificare un file studenti?','
La modifica di un file studenti già presente può essere eseguita esclusivamente dall\'autore o dai referenti dell\'insegnamento.
...
Se la procedura é stata completata con successo, verrà visualizzata una pagina di conferma della corretta modifica del file. Per tornare ai dettagli del file cliccare sul link \'torna ai dettagli del file\'
',1117109715,330);

INSERT INTO help (titolo,contenuto,ultima_modifica,indice) VALUES ('Come faccio ad eliminare un file studenti?
','
Come per la modifica, l\'eliminazione di un file studenti può essere eseguita esclusivamente dall\'autore o dai referenti dell\'insegnamento.
Se si è in possesso dei diritti per effettuare questa operazione, bisogna cliccare sul nome del file che si vuole eliminare all\'interno del Box \'Contributi degli studenti\'; si passerà a una pagina con le informazioni relative al file, per effettuare l\'eliminazione cliccare sul pulsante \'Elimina\'[img]https://www.universibo.unibo.it/tpl/unibo/file_del_32.gif[/img]. Verrà presentata una pagina di conferma, se si é sicuri di volere cancellare il file cliccare ancora su \'Elimina\'.
',1117109715,340);
...
','
Il procedimento per inserire un commento è il seguente:
