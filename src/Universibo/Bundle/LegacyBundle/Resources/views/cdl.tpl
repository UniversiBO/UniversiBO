...
{if $common_langCanaleMyUniversiBO != '' }
    ...
    {if $common_canaleMyUniversiBO == "remove"}
        <img src="{$common_basePath}/bundles/universibolegacy/images/esame_myuniversibo_del.gif" width="15" height="15" alt="" />&nbsp;
    {else}<img src="{$common_basePath}/bundles/universibolegacy/images/esame_myuniversibo_add.gif" width="15" height="15" alt="" />&nbsp;
...
<div class="elenco">
    ...
    {foreach name=t_ciclo from=$temp_anno.list item=temp_ciclo}
        ...
        {foreach name=elenco_ins from=$temp_ciclo.list item=temp_ins}
            ...
            <a href="{$temp_ins.uri|escape:"htmlall"}">{$temp_ins.name|escape:"htmlall"} - {$temp_ins.nomeDoc|lower|ucwords|escape:"htmlall"}</a>{if $temp_ins.forumUri != ''}&nbsp;&nbsp;<a href="{$temp_ins.forumUri|escape:"htmlall"}">(<img src="{$common_basePath}/bundles/universibolegacy/images/forum_omini_piccoli.gif" width="11" height="12" alt="{$cdl_langGoToForum|escape:"htmlall"}" border="0"/>&nbsp;forum)</a>{/if} {if $temp_ins.editUri != ''}&nbsp;&nbsp;<a href="{$temp_ins.editUri|escape:"htmlall"}">(modifica didattica)</a>{/if}</p>
        {/foreach}
        </div>      
    {/foreach} 
</div> 
