...
//					if ($user->isGroupAllowed($permessi_download))
                    ...
                    for ($j = 0; $j < $num_canali; $j++) {
                        ...
                        if ($canale->isGroupAllowed($user->getLegacyGroups())) {
                            ...
                            $file_tpl[$i]['download_uri'] = $router->generate('universibo_legacy_file_download', ['id_file' => $file->getIdFile(), 'id_canale' => $id_canale = $canale->getIdCanale()]);
                            $file_tpl[$i]['show_info_uri'] = $router->generate('universibo_legacy_file', ['id_file' => $file->getIdFile(), 'id_canale' => $id_canale]);
