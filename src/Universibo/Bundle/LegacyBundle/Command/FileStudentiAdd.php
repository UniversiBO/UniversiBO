...
class FileStudentiAdd extends UniversiboCommand
{
    public function execute(Request $request)
    {
        ...
        if ($canale->getServizioFilesStudenti() == false)
            Error::throwError(_ERROR_DEFAULT, ['id_utente' => $user->getId(),
        ...
        if (array_key_exists('f23_submit', $_POST)) {
            ...
            //titolo
            if (strlen($request->request->get('f23_titolo')) > 150) {
                ...
                $f23_accept = false;
            } elseif ($request->request->get('f23_titolo') == '') {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_titolo = $request->request->get('f23_titolo');

            //abstract
            $f23_abstract = $request->request->get('f23_abstract');

            $checkdate_ins = true;
            //data_ins_gg
            if (!preg_match('/^([0-9]{1,2})$/', $request->request->get('f23_data_ins_gg'))) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_data_ins_gg = $request->request->get('f23_data_ins_gg');

            //f23_data_ins_mm
            if (!preg_match('/^([0-9]{1,2})$/', $request->request->get('f23_data_ins_mm'))) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_data_ins_mm = $request->request->get('f23_data_ins_mm');

            //f23_data_ins_aa
            if (!preg_match('/^([0-9]{4})$/', $request->request->get('f23_data_ins_aa'))) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_accept = false;
                $checkdate_ins = false;
            } elseif ($request->request->get('f23_data_ins_aa') < 1970
                    || $request->request->get('f23_data_ins_aa') > 2032) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_data_ins_aa = $request->request->get('f23_data_ins_aa');

            //f23_data_ins_ora
            if (!preg_match('/^([0-9]{1,2})$/', $request->request->get('f23_data_ins_ora'))) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                    ...
                    'template_engine' => &$template]);
                $f23_accept = false;
            } elseif ($request->request->get('f23_data_ins_ora') < 0
                    || $request->request->get('f23_data_ins_ora') > 23) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_data_ins_ora = $request->request->get('f23_data_ins_ora');

            //f23_data_ins_min
            if (!preg_match('/^([0-9]{1,2})$/', $request->request->get('f23_data_ins_min'))) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                    ...
                    'template_engine' => &$template]);
                $f23_accept = false;
            } elseif ($request->request->get('f23_data_ins_min') < 0
                    || $request->request->get('f23_data_ins_min') > 59) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_data_ins_min = $request->request->get('f23_data_ins_min');

            if ($checkdate_ins == true
                    && !checkdate($request->request->get('f23_data_ins_mm'), $request->request->get('f23_data_ins_gg'), $request->request->get('f23_data_ins_aa'))) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
            ...
            $f23_data_inserimento = mktime($request->request->get('f23_data_ins_ora'), $request->request->get('f23_data_ins_min'), "0", $request->request->get('f23_data_ins_mm'), $request->request->get('f23_data_ins_gg'), $request->request->get('f23_data_ins_aa'));

            //abstract
            if (strlen($request->request->get('f23_abstract')) > 3000) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                    ...
                    'template_engine' => &$template]);
                $f23_accept = false;
            } elseif ($request->request->get('f23_abstract') == '') {
                $f23_abstract = $f23_titolo;
            } else
                $f23_abstract = $request->request->get('f23_abstract');

            //parole chiave
            if ($request->request->get('f23_parole_chiave') != '') {
                $parole_chiave = explode("\n", $request->request->get('f23_parole_chiave'));
            ...
            //permessi_download
            if (!preg_match('/^([0-9]{1,9})$/', $request->request->get('f23_categoria'))) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                    ...
                    'template_engine' => &$template]);
                $f23_accept = false;
            } elseif (!array_key_exists($request->request->get('f23_categoria'), $f23_categorie)) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_categoria = $request->request->get('f23_categoria');

            //permessi_download
            if (!preg_match('/^([0-9]{1,3})$/', $request->request->get('f23_permessi_download'))) {
                Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                if ($request->request->get('f23_permessi_download') < 0
                        || $request->request->get('f23_permessi_download') > LegacyRoles::ALL) {
                    Error::throwError(_ERROR_NOTICE, ['id_utente' => $user->getId(),
                ...
                $f23_permessi_download = $request->request->get('f23_permessi_download');
            } else {
                if ($request->request->get('f23_permessi_download') != LegacyRoles::ALL
                        && $request->request->get('f23_permessi_download')
            ...
            //          if ($request->request->get('f23_password') != $request->request->get('f23_password_confirm'))
            //          {
            ...
            //              $f23_accept = false;
            //          }
            //          elseif($request->request->get('f23_password') != '')
            //          {
            //              $f23_password = $request->request->get('f23_password');
            //          }
...
Link: '
                        . $router
                                ->generate('universibo_legacy_file_download', ['id_file' => $newFile->getIdFile(),
                                    'id_canale' => $canale
                                    ->getIdCanale()], true)
                        . PHP_EOL . '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~';
        ...
        //$this->executePlugin('ShowTopic', ['reference' => 'filestudenti']);
