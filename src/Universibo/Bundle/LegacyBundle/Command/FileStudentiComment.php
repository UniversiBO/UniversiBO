...
class FileStudentiComment extends UniversiboCommand
{
    ...
    public function execute(Request $request)
    {
        ...
        if (array_key_exists('f26_submit', $_POST)) {
            ...
            //commento
            if (trim($request->request->get('f26_commento')) == '') {
                ...
                $f26_commento = $request->request->get('f26_commento');
            }

            //voto
            if (!preg_match('/^([0-5]{1})$/', $request->request->get('f26_voto'))) {
                Error::throwError(_ERROR_NOTICE,
                        ['id_utente' => $user->getId(),
                ...
                $f26_voto = $request->request->get('f26_voto');
