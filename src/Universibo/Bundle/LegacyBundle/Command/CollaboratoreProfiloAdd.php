...
class CollaboratoreProfiloAdd extends UniversiboCommand
{
    public function execute(Request $request)
    {
        ...
        if (array_key_exists('f36_submit', $_POST)) {
            ...
            //ruolo
            if (strlen($request->request->get('f36_ruolo')) > 150) {
                ...
                $f36_accept = false;
            } elseif ($request->request->get('f36_ruolo') == '') {
                Error::throwError(_ERROR_NOTICE,
                        ['id_utente' => $user->getId(),
                ...
                $f36_ruolo = $request->request->get('f36_ruolo');

            //intro
            if ($request->request->get('f36_intro') == '') {
                Error::throwError(_ERROR_NOTICE,
                        ['id_utente' => $user->getId(),
                ...
                $f36_intro = $request->request->get('f36_intro');

            //obiettivi
            if ($request->request->get('f36_obiettivi') == '') {
                Error::throwError(_ERROR_NOTICE,
                        ['id_utente' => $user->getId(),
                ...
                $f36_obiettivi = $request->request->get('f36_obiettivi');

            $photo = $request->files->get('f36_foto');

            //email
            if (array_key_exists('f36_email', $_POST)) {
                $f36_email = $request->request->get('f36_email');
            }

            //recapito
            if (array_key_exists('f36_recapito', $_POST)) {
                $f36_recapito = $request->request->get('f36_recapito');
            }
