    ...
    public function execute(Request $request)
    {
        ...
        if (array_key_exists('f35_submit_report', $_POST)) {
            ...
            if (!array_key_exists('f35_stato', $_POST)
                    || !array_key_exists($request->request->get('f35_stato'), $f35_stati))
            ...
            if (!array_key_exists('f35_id_username', $_POST)
                    || !array_key_exists($request->request->get('f35_id_username'),
                            $f35_collab_list))
                Error::throwError(_ERROR_DEFAULT,
                        ['id_utente' => $user->getId(),
                                'msg' => 'Il form inviato non e` valido',
            ...
            $f35_stato = $request->request->get('f35_stato');
            $f35_id_username = $request->request->get('f35_id_username');
            ...
            if (trim($request->request->get('f35_report')) != '')
                $contatto->appendReport($request->request->get('f35_report'));
